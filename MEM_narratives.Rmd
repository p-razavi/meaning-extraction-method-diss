...
```{r}
 pca_8_comp <- tetra_matrix[["rho"]] %>%  
                psych::principal(nfactors = 8)

  eight_comp_outcome <- psych::kaiser(pca_8_comp, rotate = "Varimax") %>% psych::fa.sort()

  pca_8c_pattern <- eight_comp_outcome[["loadings"]] %>% 
                as.data.frame() %>% 
                tibble::rownames_to_column("word")

  #save the df
  #write.csv(pca_8c_pattern, "mem_pca_8c_pattern_matrix.csv")
